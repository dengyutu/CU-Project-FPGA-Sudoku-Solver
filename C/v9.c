...
int main(int argc, char *argv[])
{
    ...
    while(!quit)
    {
		...
		usleep(33000);
			...
			// int row;
			// for(row = 0;row<9;row++){
			// 	printf("row %d is %d\n",row,isRepetitionInRow(row));
			// }

			// int col;
			// for(col = 0;col<9;col++){
			// 	printf("col %d is %d\n",col,isRepetitionInCol(col));
			// }
			// int square;
			// for(square = 0; square<9;square++){
			// 	printf("square %d is %d\n",square,isRepetitionInSquare(square));
			// }
				...
				for(i = 0;i<3;i++){
					for(j = 0;j<3;j++){
						if((num == s_data[9*(row+i)+col+j].value)&&((i!=l)&&(m!=j))){
							...
							// printf("%d",row+i);
							// printf("%d",col+j);
							// printf("%d",s_data[(row+l)*9+col+m].value);
							// printf("%d",s_data[(9*(row+i)+col+j)].value);
							return 1;
						}
						else{
							continue;
