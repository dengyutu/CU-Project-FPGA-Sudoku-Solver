...
/// compile with
/// gcc v5.c -o v5 -O2 -lm
/// run it will ./v5 /dev/input/event1
/// compared with v4: 1. completed the array s_data and deleted all  struct Number arrays.
/// compared with v4: 2. declare the variable which stores the pointer to the Qsys port.
///////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
//////ptr
#define addr_initial_value_ptr 0x00
    ...
    while(initialize)
    {
		...
		for(k = 0; k<81; k++){
			if(s_data[k]==1){
				...
				VGA_three(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==4){
				VGA_four(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==5){
				VGA_five(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==6){
				VGA_six(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==7){
				VGA_seven(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==8){
				VGA_eight(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==9){
				VGA_nine(Index_to_x(k),Index_to_y(k),cyan);
				}
		}
