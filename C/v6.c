...
/// compile with
/// gcc v6.c -o v6 -O2 -lm
/// run it will ./v6 /dev/input/event1
/// compared with v5: 1. added the initialization, user mode, FPGA mode, quit buttons and functionality.
/// compared with v5: 2. reconstruct the while loop and if statement logic after line 274.
///////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[])
{
	...
	VGA_text(5, 55, text_init);
	VGA_text(34, 55, text_user);
	VGA_text(52, 55, text_FPGA);
    ...
    while(!quit)
    {
		...
		if(select==1){
			if( mouse_x_click >= 37 && mouse_x_click <= 207 && 430 <= mouse_y_click && mouse_y_click <= 455){
				initialize = 0;				
			}
			else if( mouse_x_click >= 262 && mouse_x_click <= 347 && 430 <= mouse_y_click && mouse_y_click <= 455){
				user_mode = 1;
				FPGA_mode = 0;				
			}
			else if( mouse_x_click >= 405 && mouse_x_click <= 495 && 430 <= mouse_y_click && mouse_y_click <= 455){
				user_mode = 0;
				FPGA_mode = 1;				
			}
		...
		VGA_box(37,430,207,455,gray);
		VGA_box(262,430,347,455,gray);
		VGA_box(405,430,495,455,gray);
