...
/// compile with
/// gcc v4.c -o v4 -O2 -lm
/// run it will ./v4 /dev/input/event1
/// compared with v3:  1. added the array s_data which has 81 elements and each elements store the value of the sudoku starting from the upper left.
/// compared with v3:  2. completed the eraze functionality.
///////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[])
{
	...
	char text_top_row[40] = "Initialization Done!\0";
    ...
    while(initialize)
    {
		...
		if(select==1){
			if( mouse_x_click >= 400 && mouse_x_click <= 550 && 430 <= mouse_y_click && mouse_y_click <= 450){
				initialize = 0;				
			}
		else
			{
			...
			if (value != ' ' && ev[1].value == 1 && ev[1].type == 1){ // Only read the key press event
				...
				for(j = 0; j < 9; j++){
					if(mapping[j].key == ev[1].code){
						...
						if (num == 1){
							One[one_size].x = Correct_xpos(mouse_x_click);
							One[one_size].y = Correct_ypos(mouse_y_click);
							one_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 2){
							Two[two_size].x = Correct_xpos(mouse_x_click);
							Two[two_size].y = Correct_ypos(mouse_y_click);
							two_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 3){
							Three[three_size].x = Correct_xpos(mouse_x_click);
							Three[three_size].y = Correct_ypos(mouse_y_click);
							three_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 4){
							Four[four_size].x = Correct_xpos(mouse_x_click);
							Four[four_size].y = Correct_ypos(mouse_y_click);
							four_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 5){
							Five[five_size].x = Correct_xpos(mouse_x_click);
							Five[five_size].y = Correct_ypos(mouse_y_click);
							five_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 6){
							Six[six_size].x = Correct_xpos(mouse_x_click);
							Six[six_size].y = Correct_ypos(mouse_y_click);
							six_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 7){
							Seven[seven_size].x = Correct_xpos(mouse_x_click);
							Seven[seven_size].y = Correct_ypos(mouse_y_click);
							seven_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 8){
							Eight[eight_size].x = Correct_xpos(mouse_x_click);
							Eight[eight_size].y = Correct_ypos(mouse_y_click);
							eight_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
						else if (num == 9){
							Nine[nine_size].x = Correct_xpos(mouse_x_click);
							Nine[nine_size].y = Correct_ypos(mouse_y_click);
							nine_size++;
							index = Index(mouse_x_click,mouse_y_click);
							s_data[index] = num;
						}
		...
		if(eraze==1){
			printf("hello\n");
			VGA_box(Correct_xpos(mouse_x_eraze),Correct_ypos(mouse_y_eraze),Correct_xpos(mouse_x_eraze)+20,Correct_ypos(mouse_y_eraze)+20,0x0000);
		...
		for(k = 0; k<81; k++){
			if(s_data[k]==1){
				VGA_one(Index_to_x(k),Index_to_y(k),cyan);
				}
			else if(s_data[k]==2){
				VGA_two(Index_to_x(k),Index_to_y(k),cyan);
				}
		}

		VGA_box(400,430,550,455,gray);
